...
if (pathname.includes("eight.html")) {
  count = 0;
...
if (parentElements.length > 0) {
  parentElements.forEach((P, index) => {
    P.addEventListener("click", (e) => {
      if (timeExpired && pathname.includes("nine.html")) return;
      ...
      if (e.target.classList.contains("child")) {
        ...
        if (isFirstChild && pathname.includes("eight.html")) {
        ...
        } else if (isFirstChild && pathname.includes("nine.html")) {
          e.target.style.backgroundColor = "green";
          e.target.style.color = "white";
          success.play();
    ...
    if (digitalClock) {
      ...
      const updateClock = () => {
        ...
        if (timeLeft <= 0) {
          ...
          else if (pathname.includes("nine.html")) {
            if (localStorage.getItem("count") === "1") {
              setTimeout(() => {
                window.location.href = "greetOne.html";
              }, 2000);
